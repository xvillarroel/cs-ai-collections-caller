...
export const handler = async (event, context) => {  
    ...
    let matrix = convertToMatrix(rows); 
    ...
    for (let i = 0; i < matrix.length; i++){
        ...
        if (!alreadyCalled){ 
            ...
            if (response.status.search('Call initiated to') >= 0) {

                rows[i].set('Called', true);
                rows[i].set('AP Contact Number', `'+${phone2Call.toString()}`);
                await rows[i].save();

            }

        } else {
            console.log(`${phone2Call} (${matrix[i][2]}) was already called.`)
        }
